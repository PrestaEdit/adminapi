# PrestaShop Admin API — Plan D : Ressource Product

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers-extended-cc:subagent-driven-development (recommended) or superpowers-extended-cc:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Implémenter ProductResource — la ressource la plus complexe du module, couvrant le CRUD complet d'un produit PS 1.7 avec tous ses champs localisés et non-localisés.

**Architecture:** Même pattern que les plans précédents (`definition()` + CRUD + `map()`), mais `ps_product_lang` a une clé composite `(id_product, id_lang, id_shop)`, ce qui nécessite un filtre shop dans la requête lang. Les combinaisons/variantes, images, et associations feature/attribute sont hors-scope de ce plan (Plan E). Plan D = Product de base, sans la mécanique des variantes.

**Tech Stack:** PHP >=7.4, PrestaShop 1.7.6+, ObjectModel `Product`, PHPUnit ^9.

**Périmètre :** Pas de combinaisons (`ps_product_attribute`), pas d'images (`ps_image`), pas d'associations stock avancé. La quantité exposée est celle de `ps_stock_available`.
...
src/Resource/
└── Product/
    └── ProductResource.php   ← Task 1 (unique)
src/Resource/ResourceRegistry.php  ← Modifié
```
...
**Goal:** Implémenter ProductResource avec CRUD complet et tous les champs localisés de `ps_product_lang`.

**Files:**
- Create: `src/Resource/Product/ProductResource.php`
- Modify: `src/Resource/ResourceRegistry.php`

**Acceptance Criteria:**
- [ ] `GET /products` → 200 avec `{items, totalItems, offset, limit, orderBy, sortOrder}`
- [ ] `GET /products/1` → 200 avec `{productId, names, price, active, reference, ...}`
- [ ] Réponse : champ `names` localisé `{"en-US": "T-Shirt", "fr-FR": "T-Shirt"}`
- [ ] `POST /products` sans `names`, `linkRewrites` ou `price` → 422
- [ ] `POST /products` avec payload minimal → 201
- [ ] `PATCH /products/1` → 200
- [ ] `DELETE /products/1` → 204
- [ ] `DELETE /products/bulk-delete` → 204
...
- [ ] **Step 1 : Créer `src/Resource/Product/ProductResource.php`**

Note sur `ps_product_lang` : clé composite `(id_product, id_lang, id_shop)`. La requête lang filtre par `id_shop = 1` (shop par défaut) pour éviter les doublons en multistore. Cette valeur est correcte pour les installations mono-boutique et récupère les données de la boutique principale en multistore.

Note sur la quantité : `ps_stock_available` contient les stocks. Pour exposer `quantity`, on fait une sous-requête simple. En cas d'absence de stock, on retourne 0.

Note sur la visibilité : ENUM `'both'`, `'catalog'`, `'search'`, `'none'`.
Note sur la condition : ENUM `'new'`, `'used'`, `'refurbished'`.
...
class ProductResource extends AbstractResource implements ResourceInterface
{
    ...
    {
        // Fetch all lang fields for the default shop (id_shop = 1)
        // ps_product_lang has composite PK (id_product, id_lang, id_shop)
        ...
        // Stock quantity from ps_stock_available
...
Ajouter après les use statements existants :
...
```bash
php -l src/Resource/Product/ProductResource.php
vendor/bin/phpunit tests/Unit/ --testdox
```

Expected : `OK (15 tests, 21 assertions)` — 0 failure.

- [ ] **Step 4 : Commit**

```bash
git add src/Resource/Product/ src/Resource/ResourceRegistry.php
git commit -m "feat: ProductResource — core CRUD with full lang fields (no combinations)"
```
